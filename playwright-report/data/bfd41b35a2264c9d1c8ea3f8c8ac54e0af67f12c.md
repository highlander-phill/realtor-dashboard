...
> 13 |     await page.getByPlaceholder('your-team-name').fill(subdomain);
  ...
  15 |     
  16 |     await expect(page).toHaveURL(`/${subdomain}/onboarding?key=setup`);
  ...
  21 | 
  22 |     await expect(page).toHaveURL(`/${subdomain}/admin`);
  23 | 
  ...
  45 |     // This assumes a master admin user is already set up for tests
  46 |     // In a real scenario, you'd handle this through fixtures or a setup script.
  47 |     await page.getByPlaceholder('your-email@example.com').fill('phillsimpson@gmail.com');
  48 |     await page.getByPlaceholder('••••••••').fill('TestPass123!');
  ...
  56 |     
  57 |     await expect(page.getByText(`permanently delete the tenant ${subdomain} Company`)).toBeVisible();
  ...
  65 |     // Expect a redirect or a "not found" message, which in this app is handled by showing the onboarding for a new team.
  66 |     await expect(page.getByText(`Welcome to Your Team Dash`)).toBeVisible();
  67 |     await expect(page.getByText(subdomain, { exact: true })).toBeVisible();
  68 |   });
  69 | });
  70 | 
```