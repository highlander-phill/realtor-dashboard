...
export const { handlers, auth, signIn, signOut } = NextAuth((req) => {
  ...
  return {
    ...
    providers: [
      ...
      Google({
        clientId: (env.AUTH_GOOGLE_ID || process.env.AUTH_GOOGLE_ID || env.GOOGLE_CLIENT_ID || process.env.GOOGLE_CLIENT_ID || "").replace(/"/g, ""),
        clientSecret: (env.AUTH_GOOGLE_SECRET || process.env.AUTH_GOOGLE_SECRET || env.GOOGLE_CLIENT_SECRET || process.env.GOOGLE_CLIENT_SECRET || "").replace(/"/g, ""),
      }),
    ],
  }
})
