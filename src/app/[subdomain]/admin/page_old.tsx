      ...
      <div className="max-w-7xl mx-auto space-y-8">
                      ...
                      return (
                        <TableRow key={agent.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors">
                          <TableCell className="font-semibold text-slate-900 dark:text-slate-100">
                            <Link href={`/${subdomain}/agent/${agent.id}`} className="hover:text-blue-600 transition-colors flex items-center gap-2 group">
                              {agent.name}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <Card className="bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800">
            ...
            <CardContent>
              <p className="text-2xl font-bold">{data.agents.reduce((sum, a) => sum + (a.listings || 0), 0)}</p>
            </CardContent>
          </Card>
          <Card className="bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800">
            <CardHeader className="pb-2">
            ...
            <CardContent>
              <p className="text-2xl font-bold">{formatCurrency(data.agents.reduce((sum, a) => sum + (a.volumePending || 0), 0))}</p>
            </CardContent>
          </Card>
          <Card className="bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800">
            <CardHeader className="pb-2">
            ...
            <CardContent>
              <p className="text-2xl font-bold">{data.agents.reduce((sum, a) => sum + (a.closings || 0), 0)}</p>
            </CardContent>
          </Card>
        </div>
        ...
        <footer className="pt-8 border-t border-slate-200 dark:border-slate-800 flex justify-between items-center text-slate-400 text-xs">
          <p>© 2026 {data.tenant.name}. All rights reserved.</p>
          <Link href={`/${subdomain}/admin/login`} className="hover:text-slate-600 dark:hover:text-slate-200 transition-colors">
            Admin Access
          </Link>
        </footer>
